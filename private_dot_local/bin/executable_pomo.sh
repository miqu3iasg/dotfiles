#!/bin/bash
# See: https://github.com/jsspencer/pomo
...
[[ -n $POMO_FOCUS_TIME ]] && FOCUS_TIME=$POMO_FOCUS_TIME || FOCUS_TIME=50
[[ -n $POMO_BREAK_TIME ]] && BREAK_TIME=$POMO_BREAK_TIME || BREAK_TIME=10

#--- Pomodoro functions ---

function pomo_start(){
    # Start new pomo block (focus+break cycle).
...
function pomo_update(){
    ...
    running=$(pomo_stat)
    block_time=$(( (FOCUS_TIME+BREAK_TIME)*60 ))
...
function pomo_stat(){
    # Return number of seconds since start of pomo block (focus+break cycle).
...
function pomo_clock(){
    ...
    # FMM:SS indicates MM:SS left in the focus block.
    # BMM:SS indicates MM:SS left in the break block.
    if [[ -e "$POMO" ]]; then
        pomo_update
        running=$(pomo_stat)
        left=$(( FOCUS_TIME*60 - running ))
        if [[ $left -lt 0 ]]; then
...
function pomo_msg(){
    ...
    # focus or break block.  This requires a Pomodoro session to
    # have already been started...
    break_end_msg='End of break period. Time for focus!'
    focus_end_msg='End of focus period. Time for break!'
    [[ -e "$POMO" ]] || return 1
    pomo_update
    running=$(pomo_stat)
    while true; do
        ...
        sleep $left
        # pomo_stat is time from the start of the focus+block cycle.
        # 1. If switching from focus->break then stat >= running + left.
        # 2. If switching from break->focus then either
        ...
        $focus || { [[ $stat -lt $running ]] && break; }
        running=$stat
    done
    if [[ $(( stat - running - left )) -le 1 ]]; then
        if $focus; then
            send_msg "$focus_end_msg"
            sleep 1
            pomo_pause
        else
...
clock
    ...
    F indicates a focus period and a prefix of P indicates the current period is
    paused.
notify
    Raise a notification at the end of every Pomodoro focus and break block in
...
POMO_FOCUS_TIME
    Duration of the focus period in minutes.  Default: 25.
POMO_BREAK_TIME
